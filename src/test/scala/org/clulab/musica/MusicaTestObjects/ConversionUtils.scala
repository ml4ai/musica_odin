...
//      case "Switch" => switchMentionToSwitch(m).toMentionString
  ...
  def chordMentionToChord(chord: Mention): Chord = {
    ...
    val chordWords = Seq("chord", "dyad", "triad")
    //    val cardinality = chord.arguments.get("cardinality").map(_.head.text)
    var cardinality = headText("cardinality", chord)
    if (cardinality.isEmpty) {
      val trigger = chord match {
        case em: EventMention => em.trigger
        case _ => ???
      }
      if (!chordWords.exists(word => trigger.text.contains(word))) {
        cardinality = Some(trigger.text)
      }
    }
  ...
  def locationMentionToLocation(m: Mention): Location = {
    ...
    val beat = headText("beat", m).map(Beat)
    ...
    Location(locationTerm, note, measure, beat, chord, rest)
  }
  ...
  def switchMentionToSwitch(s: Mention): Switch = {
    val sourceEntity = s.arguments.get("sourceEntity").map(se => musicalEntityMentionToMusicalEntity(se.head))
    val sourceLocation = s.arguments.get("sourceLocation").map(sl => locationMentionToLocation(sl.head))
    val destEntity = s.arguments.get("destEntity").map(de => musicalEntityMentionToMusicalEntity(de.head))
    val destLocation = s.arguments.get("destLocation").map(dl => locationMentionToLocation(dl.head))
